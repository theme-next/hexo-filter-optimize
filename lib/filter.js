...
module.exports = function() {
  ...
  var { remove_query_string, remove_comments, css, js } = config;
      ...
      return streamRead(stream)
        .then(str => {
          ...
          if (remove_query_string) {
            links
              .filter(el => regQuery.test(el.href))
              .forEach(el => el.href = el.href.replace(regQuery, ''));
          }
          if (css.enable) {
            links
          ...
          if (js.bundle || remove_query_string) {
            var scriptText = null;
            [...doc.querySelectorAll('script')].forEach(el => {
              var src = el.src;
              if (remove_query_string && regQuery.test(src)) {
